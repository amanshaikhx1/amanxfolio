        ...
        >
          {portfolioItems.map((item) => (
            <motion.div
              ...
              className="group bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-transform transform hover:-translate-y-2 cursor-pointer relative"
              onClick={() => isMobile ? setShowButtonId(prev => prev === item.id ? null : item.id) : openModal(item)}
            >
              ...
              {!isMobile && (
                <div className="absolute inset-0 bg-gradient-to-t from-green-500/80 to-green-400/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-300">
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      openModal(item)
        ...
        <AnimatePresence>
          {selectedItem && (
            ...
            >
              ...
              >
                <button
                  className="absolute top-4 right-4 w-8 h-8 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md text-black hover:bg-white/30 transition-all"
                  onClick={closeModal}
                ...
                <div className="flex flex-col md:flex-row gap-6 p-6 overflow-y-auto">
                  ...
                  <div className="md:w-1/2 w-full space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      {/* <div>
                        <p className="text-green-500 font-medium">Client:</p>
                        <p className="text-gray-600 dark:text-gray-300">{selectedItem.client}</p>
                      </div> */}
                      <div>
                    ...
                    {selectedItem.link && (
                      <a
                        ...
                        className="inline-block mt-4 px-6 py-3 bg-gradient-to-r from-green-500 to-green-400 text-white rounded-full shadow hover:from-green-600 hover:to-green-500 transition-transform transform hover:-translate-y-1"
                      >
                        View Project
                      </a>
                    )}
