...
const ResumeItem = memo(({ data, index, type }: { data: any; index: number; type: "education" | "experience" }) => {
  return (
    <motion.div
    ...
    >
      <div className="absolute left-0 top-0 h-full w-px bg-gradient-to-b from-green-500 to-transparent"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16">
          ...
          <div>
            ...
            {educationData.map((item, index) => (
              <ResumeItem key={`edu-${index}`} data={item} index={index} type="education" />
            ))}
          </div>
          ...
          <div>
            ...
            {experienceData.map((item, index) => (
              <ResumeItem key={`exp-${index}`} data={item} index={index} type="experience" />
            ))}
          </div>
        </div>
      </div>
